...
const handleUnsubscribe = httpAction(async (ctx, request) => {
  ...
  const ok = await ctx.runQuery(internal.articleEmail.verifyUnsubscribeToken, {
    clerkId,
    token,
  })
  if (!ok) {
  ...
  if (!updated) {
    return new Response('Conta não encontrada.', {
      status: 404,
      headers: { 'Content-Type': 'text/plain; charset=utf-8' },
    })
  }
...
http.route({ path: '/api/unsubscribe', method: 'GET', handler: handleUnsubscribe })
http.route({ path: '/api/unsubscribe', method: 'POST', handler: handleUnsubscribe })
