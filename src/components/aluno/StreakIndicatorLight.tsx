...
// Variante light theme do StreakIndicator. Usada em headers light
// (AulaPage, CursoInternoPage) onde o aluno passa tempo estudando.
// Mostra streak de dias consecutivos. Esconde quando o aluno ainda não
// concluiu nenhuma aula.

type Props = { compact?: boolean }

export function StreakIndicatorLight({ compact = false }: Props) {
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        alive && streak > 0
          ? 'border-[#F37E20]/30 bg-[#F37E20]/8 text-[#B5560F] hover:bg-[#F37E20]/12'
          : 'border-gray-200 bg-white text-gray-500 hover:border-gray-300',
      )}
    >
      <svg
        className={cn('h-3.5 w-3.5', alive && streak > 0 ? 'text-[#F37E20]' : 'text-gray-400')}
        fill="currentColor"
        viewBox="0 0 24 24"
      >
      ...
      {!compact && <span className="text-gray-500">{streak === 1 ? 'dia' : 'dias'}</span>}
    </Link>
  )
}
