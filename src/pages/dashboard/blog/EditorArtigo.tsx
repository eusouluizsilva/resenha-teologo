...
import { SeletorIdentidade, type IdentityValue } from '@/components/blog/SeletorIdentidade'
...
export function EditorArtigo({ mode }: ArticleEditorProps) {
    ...
    <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="space-y-6">
      ...
      <motion.div variants={fadeUp} className={cn('space-y-6 p-6 sm:p-7', brandPanelClass)}>
        <div className="space-y-2">
          ...
          <SeletorIdentidade value={identityValue} onChange={setIdentityValue} disabled={busy} />
        </div>
      ...
      <motion.div variants={fadeUp} className={cn('space-y-3 p-6 sm:p-7', brandPanelClass)}>
        ...
        {showPreview ? (
          <div className="rounded-2xl border border-white/8 bg-[#F7F5F2] px-6 py-6 text-[#111827]">
            <CorpoArtigo markdown={body || '*(vazio)*'} />
          </div>
        ) : (
          <textarea
            id={`${formId}-body`}
