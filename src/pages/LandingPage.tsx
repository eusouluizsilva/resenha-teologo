...
export function LandingPage() {
  ...
  const featuredArticles = useQuery(api.postRanking.getTopByCategory, {
    categorySlug: 'all',
    limit: 3,
  })
            ...
            <motion.div variants={fadeUp} className="mb-12 flex flex-wrap items-end justify-between gap-4">
              <div>
                ...
                <h2 className="mt-3 font-display text-3xl font-bold leading-tight md:text-4xl">
                  Leituras que estão circulando agora.
                </h2>
                <p className="mt-3 max-w-xl font-serif text-base leading-7 text-[#4B5563]">
                  Artigos publicados por professores, alunos e instituições. Sem paywall, sem ruído.
                </p>
              </div>
              <Link
                to="/blog"
            ...
            <motion.div variants={staggerContainer} className="grid grid-cols-1 gap-6 md:grid-cols-3">
              {featuredArticles.map((article) => (
                <motion.div key={String(article._id)} variants={fadeUp}>
                  ...
                  >
                    <div className="aspect-[16/9] w-full overflow-hidden bg-[#F2EAE1]">
